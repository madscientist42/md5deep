...
void display::status(const char *fmt,...)
{
    va_list(ap);
    va_start(ap, fmt);
    if(vsnprintf(msgBuf, sizeof(msgBuf) - 1, fmt, ap) < 0)
    {
...
void display::error(const char *fmt,...)
{
    ...
    lock();
    std::cerr << progname << ": " << msgBuf << std::endl;
    unlock();
    va_end(ap);
}
...
void display::fatal_error(const char *fmt,...)
{
    ...
    writeln(&std::cerr,progname + ": " + msgBuf);
    va_end(ap);
    exit(1);
}
...
void display::print_debug(const char *fmt, ... )
{
    ...
    writeln(&std::cerr,std::string("DEBUG:" ) + msgBuf);
    va_end(ap);
#endif
}
/**
...
void display::error_filename(const std::string &fn,const char *fmt, ...)
{
    ...
    if(dfxml)
    {
		...
		dfxml->xmlout("error", msgBuf);
		dfxml->pop();
		unlock();
    }
    else
    {
		writeln(&std::cerr,fmt_filename(fn) + ": " + msgBuf);
    }
    va_end(ap);
}
...
void file_data_hasher_t::dfxml_write_hashes(std::string hex_hashes[],int indent)
{
    for(int i=0;i<NUM_ALGORITHMS;i++)
    {
		if(hashes[i].inuse)
		{
			...
			this->dfxml_hash << "<hashdigest type='" << makeupper(hashes[i].name) << "'>" << hex_hashes[i] <<"</hashdigest>\n";
		}
    }
}
...
void file_data_hasher_t::compute_dfxml(bool known_hash,const hash_context_obj *hc)
{
    ...
    {
		this->dfxml_hash << "<byte_run file_offset='" << hc->read_offset << "'" << " len='" << hc->read_len << "'>   \n";
		indent=2;
    }
...
int display::audit_check()
{
    ...
    return ((0 == this->match.unused) && (0 == this->match.unknown) && (0 == this->match.moved));
}
